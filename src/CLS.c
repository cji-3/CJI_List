...
CLS_List CLS_CreateL_P(size_t DataByte,float Increment){
	...
	_debug(COMMON,"CreateList_Whole success. list:%p",_return);
	return (CLS_List)_return;
}
...
int CLS_AddLast(CLS_List list,void* data){
	if(!_IsExpansion(list,1)){	//不需要擴容
		...
		_debug(COMMON,"Add success. list:%p, UsedByte:%zu/%zu",list,list->UsedByte,list->ComByte);
		return 0;
	}
	else{	//需要擴容
		...
		if(rp!=NULL){	//成功
			...
			_debug(SIGN,"Add success and occur expansion. list:%p, OldComByte:%zu, NewUsedByte:%zu/%zu",list,list->ComByte,list->UsedByte,_Byte);
...
int CLS_AddIndex(CLS_List list,size_t index,void* data){
	...
	//index check
	if(index > len){
		_debug(ERROR,"AddIndex error. AddIndex out of bounds. list:%p, index:%zu, len:%zu",list,index,len);
		return -1;
	}
	...
	if(!_IsExpansion(list,1)){     // 不需要擴容
		...
		_debug(COMMON,"AddIndex success. list:%p, index:%zu, UsedByte:%zu/%zu",list,index,list->UsedByte,list->ComByte);
		return 0;
	}
	else{	//需要擴容
		size_t _Byte;	//擴容後的總位元組數
		...
		if(rp !=NULL){	//成功
			...
			_debug(SIGN,"AddIndex success and occur expansion. list:%p, UsedByte:%zu/%zu",list,list->UsedByte,list->ComByte);
...
void CLS_Replace(CLS_List list,size_t index,void* data){
	_Write(list,index,data);
	_debug(COMMON,"Replace success. list:%p, index:%zu",list,index);
}

//讀取list中的元素
void* CLS_ReadIndex(CLS_List list,size_t index){
	if(index*list->DataByte < list->UsedByte){
		_debug(COMMON,"Read success. list:%p, index:%zu",list,index);
		return list->Address + index*list->DataByte;
	}
	else{
		_debug(ERROR,"Read error. Read out of bounds. list:%p, index:%zu, UsedIndex:%zu",list,index,list->UsedByte/list->DataByte);
		return NULL;
	}
}
...
int CLS_DelIndex(CLS_List list,size_t index){
	...
	if(index < len){
		...
		_debug(COMMON,"Delete success. list:%p, index:%zu, UsedByte:%zu/%zu",list,index,list->UsedByte,list->ComByte);
		return 0;
	}
	else{
		_debug(ERROR,"Delete error. Delete out of bounds. list:%p, index:%zu, len:%zu",list,index,len);
		return -1;
	}
}
